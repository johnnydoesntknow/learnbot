  ...
  useEffect(() => {
    const initializeDashboard = async () => {
      try {
        ...
        // Fetch user progress from API (NEW ENDPOINT)
        const progressRes = await fetch('http://localhost:3001/api/user/progress', {
          headers: token ? { 'Authorization': `Bearer ${token}` } : {}
        });
        const progressData = await progressRes.json();
        
        // Set states from database
        setCompletedQuizzes(progressData.completedQuizzes || []);
        setFailedQuizzes(progressData.failedQuizzes || []);
        setTotalRepPoints(progressData.repPoints || 0);
        setUserLevel(progressData.level || 1);
        
        // Fetch lessons
        const lessonsRes = await fetch('http://localhost:3001/api/lessons', {
          headers: token ? { 'Authorization': `Bearer ${token}` } : {}
        });
        const lessonsData = await lessonsRes.json();
        
        // Map completed/failed status from database progress
        const lessonsWithStatus = lessonsData.map(lesson => ({
          ...lesson,
          completed: progressData.completedQuizzes.includes(lesson.id),
          failed: progressData.failedQuizzes.includes(lesson.id)
        }));
        
        setLessons(lessonsWithStatus);
        
        // Fetch REP configuration
        ...
        // Set default lessons if API fails
        setLessons([
          { id: 'lesson-1', title: 'Lesson 1', description: 'Error loading', contentType: 'video', duration: '5 min', completed: false, failed: false },
          { id: 'lesson-2', title: 'Lesson 2', description: 'Error loading', contentType: 'text', duration: '8 min', completed: false, failed: false },
          { id: 'lesson-3', title: 'Lesson 3', description: 'Error loading', contentType: 'interactive', duration: '10 min', completed: false, failed: false }
        ]);
      } finally {
        setLoading(false);
      }
