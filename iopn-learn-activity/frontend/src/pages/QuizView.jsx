...
const QuizView = ({ user }) => {
  ...
  useEffect(() => {
    const checkAttemptsAndFetch = async () => {
      try {
        ...
        // Check if ANY quiz is active in another tab (keep this for multi-tab prevention)
        ...
        // Fetch quiz data AND attempt count from DATABASE
        ...
        // USE ONLY THE DATABASE ATTEMPT NUMBER
        const dbAttemptNumber = data.attemptNumber || 1;
        const actualAttemptsUsed = dbAttemptNumber - 1; // Convert next attempt # to used attempts
        
        console.log(`Database says: Next attempt will be #${dbAttemptNumber}, so ${actualAttemptsUsed} attempts used`);
        
        // Check if already at or over 3 attempts based on DATABASE
        if (actualAttemptsUsed >= 3) {
        ...
        setAttempts(actualAttemptsUsed);
  ...
  const submitQuiz = async (finalAnswers) => {
    try {
      ...
      if (!response.ok) {
        ...
        // Parse error to check if it's max attempts
        try {
          const errorData = JSON.parse(errorText);
          if (errorData.error === 'Maximum attempts exceeded') {
            setBlockMessage('Maximum attempts exceeded. This quiz is now locked.');
            setShowBlockModal(true);
            setTimeout(() => {
              navigate('/');
            }, 3000);
      ...
      // No more localStorage updates - dashboard will fetch fresh data from database
  ...
  if (showBlockModal) {
    const isExceededAttempts = blockMessage?.includes('exceeded') || blockMessage?.includes('Maximum');
    return (
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#0a0a0b]">
        <div className="text-center">
          ...
          <button
            onClick={startQuiz}
            disabled={attempts >= 3}
            className={`px-8 py-3 rounded-lg font-semibold text-white transition-transform ${
              attempts >= 3 
                ? 'bg-gray-600 cursor-not-allowed opacity-50'
                : 'bg-gradient-to-r from-[#6105b6] to-[#2280cd] hover:scale-105'
            }`}
          >
            {attempts >= 3 ? 'No Attempts Remaining' : 'Start Quiz'}
          </button>
            ...
            <>
              ...
              <p className="text-white/60 text-sm mb-6">
                {results.attemptNumber < 3 
                  ? `You have ${3 - results.attemptNumber} attempt${3 - results.attemptNumber === 1 ? '' : 's'} remaining.` 
