    ...
    try {
      ...
      if (user.length > 0) {
        const attempts = await db.query(
          'SELECT COUNT(*) as count FROM quiz_attempts WHERE user_id = ? AND lesson_id = ?',
          [user[0].id, lessonId]
        );
...
const dbLessonId = lessons[0].id;  // This is the integer ID (1, 2, or 3)

// Get quiz ID for this lesson
const quizzes = await db.query(
  'SELECT id FROM quizzes WHERE lesson_id = ?',
  [dbLessonId]
);

const dbQuizId = quizzes[0]?.id || 1;

// Get attempt number using dbLessonId
const attempts = await db.query(
  'SELECT COUNT(*) as count FROM quiz_attempts WHERE user_id = ? AND lesson_id = ?',
  [user.id, dbLessonId]  // Use dbLessonId here
);
...
await db.query(
  ...
  [user.id, dbQuizId, dbLessonId, attemptNumber, JSON.stringify(answers), 
   correct, percentage, passed, 0]  // Use dbLessonId NOT lessonId
);
...
if (passed && points > 0) {
  ...
  await db.query(
    ...
    [user.id, dbLessonId]  // Use dbLessonId NOT lessonId
  );
  ...
  await db.query(
    ...
    [user.id, dbQuizId, dbLessonId, correct, total, percentage, passed, points]  // Use dbLessonId
  );
}
    
    console.log(`📊 Saved to MySQL - User: ${user.username}, Lesson: ${lessonId}, Attempt: ${attemptNumber}, Score: ${percentage}%, REP: ${points}`);
